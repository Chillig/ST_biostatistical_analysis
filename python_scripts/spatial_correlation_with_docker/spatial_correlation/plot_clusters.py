...
def plot_clusters_counts(sub_adata, sup_cluster_adata, cyto, save_folder, obs_label, obs_counts):
    """

    :param sub_adata: [dict] containing annData object
    :param sup_cluster_adata:
    ...
    sample = sup_cluster_adata.obs["sample"].cat.categories[0]
    specimen = sup_cluster_adata.obs["specimen"].cat.categories[0]
    # if specimen in samples:
    # ind = samples.index(specimen)

    # temp_adata = sup_adata[sup_adata.obs["specimen"] == specimen]
    temp_adata = sup_cluster_adata[sup_cluster_adata.obs["sample"] == sample]
    ...
    if test_counts.shape[0] > 0:
        ...
        # Draw only H&E image
        sc.pl.spatial(sub_adata, size=0, img_key=img_key, library_id=library_id_temp,
        ...
        leg = ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.1), ncol=3, fontsize=legend_fontsize)
