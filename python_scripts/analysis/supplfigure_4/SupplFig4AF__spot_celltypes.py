...
class DeconvolutionAnalysis:
    def __init__(self, save_folder, adata_st, adata_mapping, cytokine, sample_name):
        ...
        # Get mask to keep only cyto+ spots
        cyto_columns = list(self._adata_st.obs.columns[
                                self._adata_st.obs.columns.str.contains('_{}'.format(cytokine[sample_name]))])
        self._valid = self._adata_st.obs[cyto_columns].values == cytokine[sample_name]
    ...
    def adata_st(self):
        ...
        return self._adata_st[self._valid].copy()

    @property
    def adata_mapping(self):
        return self._adata_mapping[:, self._valid].copy()
    ...
    def sort_subcelltypes_by_tissuelayer(self, df, subcelltypes, cytokine, sample_name):
        df['Tissue_layer'] = self.df_map_celltypes.loc['Epi-Dermis'][:-1][list(df.index)]
        df.to_excel(
            os.path.join(self.save_folder, 'Tangram_{}_sample{}__{}.xlsx'.format(
                cytokine[sample_name], sample_name, subcelltypes)))
...
def merge_labels(df_map):
    ...
    for col in col_names:
        ...
        # Use grouping like in HaniffaLab paper
        df_map_celltypes.loc['APCs', col] = df_map.loc[
            df_map.index.str.contains('DC[0-9]|Inf_mac|LC_[0-9]|Macro|MigDC|Mono_mac|moDC'), col].sum()
        df_map_celltypes.loc['Lymphoid & Mast', col] = df_map.loc[
            df_map.index.str.contains('ILC1_3|NK|ILC2|Tc|TcIL13_IL22|Tc17_Th17|Th|Treg|Mast_cell|Plasma'), col].sum()
        df_map_celltypes.loc['Dermal non-immune', col] = df_map.loc[
            df_map.index.str.contains('Schwann|F[0-9]|Pericyte|VE|LE[0-9]'), col].sum()
        df_map_celltypes.loc['Epidermal non-immune', col] = df_map.loc[
            df_map.index.str.contains('Melanocyte|KC'), col].sum()
...
def plot_annotate_spot_by_most_occuring_celltype(adata, sample_name, save_folder):
    df_test = adata.obsm['tangram_ct_pred']
    df_occurency = pd.DataFrame(
        pd.DataFrame({'Cell type spot': df_test.columns[np.argmax(np.asarray(df_test), axis=1)]})[
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_folder, '{}__Most_occuring_celltype_in_cytopos_spot.png'.format(sample_name)))
    plt.close(fig=fig)


def plot_sortedbymajority_celltype_frequency(df_map_celltypes, save_folder, sample_name):
    df_map_celltypes_temp = df_map_celltypes[df_map_celltypes.columns[df_map_celltypes.columns != 'colors']]
    df_occurency = pd.DataFrame(
        {'Cell type spot': df_map_celltypes_temp.index[np.argmax(np.asarray(df_map_celltypes_temp), axis=0)]})[
        'Cell type spot'].value_counts()
    df_occurency = df_occurency.reset_index()
    df_occurency.columns = ['Cell type', 'Frequency']
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.barplot(data=df_occurency, x='Cell type', y='Frequency', ax=ax)
    ax.set_xticklabels(df_occurency['Cell type'].values, rotation=90, ha='right')
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_folder, '{}__Majoritycelltype__Most_occuring_celltype_in_spot.png'.format(sample_name)))
    plt.close(fig=fig)
...
def main(save_folder):
    ...
    # input_dir = '/Volumes/CH__data/ST_immune_publication/Revision/adata/2022-08-10'
    ...
    for ind_sample, sample_name in enumerate(samples):
        ...
        deconv_obj = DeconvolutionAnalysis(save_folder=save_folder, adata_st=st_adata, adata_mapping=adata_mapping,
                                           cytokine=cytokine, sample_name=sample_name)
        ...
        df_lymphoid_mastcells, df_colors_old = deconv_obj.get_subcelltypes(subcelltypes='Lymphoid & Mast')
        ...
        df_lymphoid_mastcells_grouped = deconv_obj.sort_subcelltypes_by_tissuelayer(
            df=df_lymphoid_mastcells_grouped, subcelltypes='Lymphoid & Mast',
            cytokine=cytokine, sample_name=sample_name)
        ...
        df_apcs_grouped = deconv_obj.sort_subcelltypes_by_tissuelayer(
            df=df_apcs_grouped, subcelltypes='APCs', cytokine=cytokine, sample_name=sample_name)
        # sort columns of df_color by columns of other dataframe
        ...
        deconv_obj.plot_barcharts(
            ...
            order=[list(df_apcs_grouped.columns).index('DC'),
                   list(df_apcs_grouped.columns).index('Mo_DC'),
                   list(df_apcs_grouped.columns).index('Macro'),
                   list(df_apcs_grouped.columns).index('Mono_mac'),
                   list(df_apcs_grouped.columns).index('LC')])
        # Save Groups to excel file
        df_apcs_grouped.to_excel(os.path.join(save_folder, 'APCs.xlsx'))

        # Plot occurency of subcell types
        plot_sortedbymajority_celltype_frequency(df_map_celltypes[:-1], save_folder, sample_name)

        # Plot H&E image + Pie charts
        patches = deconv_obj.plot_he_celltype_piecharts(
            st_adata=st_adata, sample_name=sample_name, ind_sample=ind_sample, df_map_celltypes=df_map_celltypes[:-1])

        print('Plot Barplot')
        # index are the points, columns are the cell types
        plot_unsorted_bargraph(
            df=df_map_celltypes[:-1], df_map=deconv_obj.df_map, patches=patches,
            sample_name=sample_name, save_folder=save_folder)
        ...
        # Save colors
        colors = sc.pl.palettes.zeileis_28[:df_map_celltypes.shape[0] - 1]
        df_colors = pd.DataFrame(dict(zip(list(df_map_celltypes.index[:-1]), colors)), index=[0])
