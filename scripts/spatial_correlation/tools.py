...
def subset_adata(adata, marker_genes, observable):
    """Remove spots without any tissue type label and subset adata to cell types of interest

    Parameters
    ----------
    adata : annData
    marker_genes : str, list
    observable : str
        observable name
    ...
    adata = adata[adata.obs[observable] != 'Unknown']
    # Subset spots to only spots which contain marker genes
    m_subspots = get_celltypes_data(adata=adata, genes=marker_genes)
    sub_adata = adata[m_subspots]
...
def get_celltypes_data(adata, genes):
    ...
    if isinstance(genes, list):
        varindex_genes = np.where(adata.var.index[np.newaxis, :] == np.array(genes)[:, np.newaxis])[1]
        counts_gene = adata.layers["counts"][:, varindex_genes]
    else:
        varindex_genes = np.where(adata.var.index == genes)[1]
        counts_gene = adata.layers["counts"][:, varindex_genes][:, 0]
    # create mask
    m_gene = counts_gene > 0
    m_gene = np.any(m_gene, axis=1)
...
def add_genecomb_column(adata, gene_combs, label):
    """Ass classifier genes with cytokine and others spot on tissue slice
        others label: 0
        cytokine label: >= 1
    ...
    ----------
    adata : annData
    gene_combs : str, list, dict
        contains gene combination or gene
    label : str
    ...
    # set default label "Others"
    default_label = np.array(['Others'] * adata.shape[0], dtype='<U64')
    default_clusters = np.array([0] * adata.shape[0])

    if isinstance(gene_combs, dict):
        for ind, comb in enumerate(gene_combs.keys()):
            # get masks for unique combinations and count matrix
            m_genes, matrix = unique_combs(adata, genes=gene_combs[comb])
            # 3.1 Get gene spot position
            # Add label of spots
            default_label[m_genes] = label
            default_clusters[m_genes] = 0
            default_counts[m_genes] = np.sum(matrix, axis=1)[m_genes]

    else:
        m_genes, matrix = unique_combs(adata=adata, genes=gene_combs)
...
def unique_combs(adata, genes):
    """Identify positions of unique gene combinations and read out counts
    ...
    # 1. Index of genes
    varindex_genes = np.where(adata.var.index[np.newaxis, :] == np.array(genes)[:, np.newaxis])[1]
    # 2.1 Get cytokine spot positions
    if "counts" in adata.layers.keys():
        matrix = adata.layers['counts'][:, varindex_genes].copy()
    else:
        matrix = adata.X[:, varindex_genes].copy()
    m_counts = matrix.astype(bool)

    # 2.2 Find combinations of genes
    comb_genes = np.where(m_counts, np.array(genes), '')

    # Get mask of unique combination of genes --> check for list of genes
    m_genes = comb_genes == genes
    m_genes = np.all(m_genes, axis=1)

    return m_genes, matrix
