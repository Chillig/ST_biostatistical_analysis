...
df.metaData <- as.data.frame(adata.obs[c('sample', 'project', 'patient', 'DISEASE', 'biopsy_type', 
                           'tissue_layer', 'size_factors', 'specimen', 'IL17A_in_sdcc_r0')])
row.names(df.metaData) <- unique.barcodes
# Design
design.func = ~cdr + project + patient + tissue_layer + IL17A_in_sdcc_r0

# Remove nn spots label = 2
mask.nn <- df.metaData$IL17A_in_sdcc_r0 == 2
...
print(df.dge[df.dge$gene_symbol == 'IL17A',  ])

write.csv(df.dge, file="/Users/christina.hillig/PycharmProjects/ST_Immune_publication/Publication_analysis/output/reviewers/Figure_S8/optional/glmGamPoi_DGE_analysis/DEGs_IL17A_optimal_radius0.csv")
