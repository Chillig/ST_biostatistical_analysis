...
df.metaData <- as.data.frame(adata.obs[c('sample', 'project', 'patient', 'DISEASE', 'biopsy_type', 
                           'tissue_layer', 'size_factors', 'specimen', 'IL13_in_sdcc_r3')])
row.names(df.metaData) <- unique.barcodes
# Design
design.func = ~cdr + project + patient + tissue_layer + IL13_in_sdcc_r3

# Remove nn spots label = 2
mask.nn <- df.metaData$IL13_in_sdcc_r3 == 2
...
print(df.dge[df.dge$gene_symbol == 'IL13',  ])

write.csv(df.dge, file="/Users/christina.hillig/PycharmProjects/ST_Immune_publication/Publication_analysis/output/reviewers/Figure_S8/optional/glmGamPoi_DGE_analysis/DEGs_IL13_optimal_radius3.csv")
